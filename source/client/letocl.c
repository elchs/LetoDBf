...
#endif
{
   ...
   if( LETO_MSGSIZE_LEN + ulLen + 9 > pConnection->ulBufferLen )
   {
      pConnection->ulBufferLen = LETO_MSGSIZE_LEN + ulLen + 9;
      pConnection->szBuffer = ( char * ) hb_xrealloc( pConnection->szBuffer, pConnection->ulBufferLen + 1 );
   }
...
static long leto_SendRecv2( LETOCONNECTION * pConnection, const char * szData, unsigned long ulLen, int iErr )
{
   ...
   {
      lRet = leto_Send( pConnection, szData, ulLen );
   ...
   {
      lRet = leto_DataSendRecv( pConnection, szData, ulLen );
...
static _HB_INLINE_ long leto_SendRecv( LETOCONNECTION * pConnection, const char * szData, unsigned long ulLen, int iErr )
{
   long lRet;
...
HB_EXPORT HB_ERRCODE LetoDbGoTo( LETOTABLE * pTable, unsigned long ulRecNo )
{
...
#if 0  /* no limit receive */
         if( ! ulRecLen || ulRecLen >= 1000000 )  // 2^24 = 16777215
            break;
#endif
         if( HB_GET_LE_UINT32( ptrBuf + 3 ) == ulRecNo )
         {
   ...
   if( ! fFound )
   {
      ...
      else
      {
         ...
         leto_ParseRecord( pConnection, pTable, ptr, HB_TRUE );
         if( ! pTable->fEof )
            leto_setSkipBuf( pConnection, pTable, ptr, 0 );
      }
      pTable->ptrBuf = NULL;
   }
   ...
   if( pTable->ptrBuf && leto_HotBuffer( pTable, &pTable->Buffer, pConnection->iBufRefreshTime ) )
   {
      ...
      else
      {
         ...
         else if( pTable->uiRecInBuf >= ulSkips )
         {
            ...
            if( ul == pTable->uiRecInBuf )
               ul = ulSkips;
         }

         if( ul == ulSkips )
         {
            if( HB_GET_LE_UINT24( pTable->ptrBuf ) )
            {
               leto_ParseRecord( pConnection, pTable, ( char * ) pTable->ptrBuf, HB_FALSE );
...
HB_EXPORT HB_ERRCODE LetoDbRecLock( LETOTABLE * pTable, unsigned long ulRecNo )
{
   ...
   if( ! pTable->fShared || ( ulRecNo == pTable->ulRecNo ? pTable->fRecLocked : leto_IsRecLocked( pTable, ulRecNo ) ) )
      return 0;
   leto_ClearBuffers( pTable );  // ToDo moved this below above check
...
HB_EXPORT HB_ERRCODE LetoDbRecUnLock( LETOTABLE * pTable, unsigned long ulRecNo )
{
   ...
   if( ! pTable->fShared || pTable->fFLocked ||
      ...
      return 0;
   leto_ClearBuffers( pTable );  // ToDo moved this below above checks
