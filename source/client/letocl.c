...
#if defined( HB_OS_WIN ) && defined( LETO_DYNLIB )

/* minGw may need libgcc_s_dw2-1.dll + libwinpthread-1.dll at place of exe */
BOOL WINAPI DllMain( HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved )
{
   static int s_fHVMIsInit = 0;
   ...
   switch( fdwReason )
   {
      case DLL_PROCESS_ATTACH:
         if( ! s_fHVMIsInit )
         {
            ...
            MessageBox( 0, "We entered DLLMain", "1", 0 );
   #endif
            s_fHVMIsInit = 1;
         }
         break;
      ...
      case DLL_PROCESS_DETACH:
         ...
         {
   #ifdef LETO_CLIENTLOG
            MessageBox( 0, "We exit DLLMain", "1", 0 );
   #endif
            s_fHVMIsInit = 0;
         }
         break;
...
#endif  /* HB_OS_WIN && LETO_DYNLIB */

LETOCONNECTION * LetoConnection( unsigned int uiConnection, LETOTABLE * pTable )
{
   ...
   if( pTable )
      ...
      pConnection = letoGetConnPool( uiConnection );
...
static char * leto_strdup( const char * pszText )
{
   HB_SIZE nLen = strlen( pszText ) + 1;
   char *  pszDup = ( char * ) hb_xgrab( nLen );

   memcpy( pszDup, pszText, nLen );
   return pszDup;
}

static void leto_strncpy( char * pDest, const char * pSource, HB_SIZE nLen )
{
   pDest[ nLen ] = '\0';
   ...
   void LetoSetGetSoftSeek( unsigned int * iSet )     { *iSet = ( int ) s_pLetoSetGet->SoftSeek; }
   void LetoSetGetDeleted( unsigned int * iSet )      { *iSet = ( int ) s_pLetoSetGet->Deleted; }
   void LetoSetGetExclusive( unsigned int * iSet )    { *iSet = ( int ) s_pLetoSetGet->Exclusive; }
   void LetoSetGetAutOpen( unsigned int * iSet )      { *iSet = ( int ) s_pLetoSetGet->AutOpen; }
   void LetoSetGetAutOrder( unsigned int * iSet )     { *iSet = ( int ) s_pLetoSetGet->AutOrder; }
   void LetoSetGetDateFormat( char * pSet )           { strcpy( pSet, s_pLetoSetGet->DateFormat ); }
   void LetoSetGetEpoch( unsigned int * iSet )        { *iSet = ( int ) s_pLetoSetGet->Epoch; }
   void LetoSetGetDefault( char * pSet )              { strcpy( pSet, s_pLetoSetGet->Default ); }
   void LetoSetGetPath( char * pSet )                 { strcpy( pSet, s_pLetoSetGet->Path ); }
   void LetoSetGetCodepage( char * pSet )             { strcpy( pSet, s_pLetoSetGet->id ); }
   ...
   void LetoSetSetAutOrder( int iSet )            { s_pLetoSetGet->AutOrder = iSet; }
   void LetoSetSetDateFormat( const char * pSet ) { leto_strncpy( s_pLetoSetGet->DateFormat, pSet, 16 - 1 ); }
   void LetoSetSetEpoch( HB_SIZE nSet )           { s_pLetoSetGet->Epoch = nSet; }
   void LetoSetSetDefault( const char * pSet )    { leto_strncpy( s_pLetoSetGet->Default, pSet, HB_PATH_MAX - 1 ); }
   void LetoSetSetPath( const char * pSet )       { leto_strncpy( s_pLetoSetGet->Path, pSet, HB_PATH_MAX - 1 ); }
   void LetoSetSetCodepage( const char * pSet )   { leto_strncpy( s_pLetoSetGet->Path, pSet, 8 - 1 ); }
#endif
...
static HB_ERRCODE leto_BackupStart( HB_BOOL fClose, int iDelay, PHB_ITEM pMsgLines );
   ...
   if( s_pError )  /* this hurts mutex, but should be ok for this quick pre-test */
   {
      ...
      if( pError )
      {
         ...
         if( errCode == LETO_CLIENT_LOCKON || errCode == LETO_CLIENT_LOCKWAIT || errCode == LETO_CLIENT_LOCKOFF )
         {
            ...
            PHB_ITEM   pMsgLines = NULL;

            if( errCode == LETO_CLIENT_LOCKON )
            {
               pMsgLines = hb_itemArrayNew( 1 );
               hb_arraySetC( pMsgLines, 1, hb_errGetOperation( pError ) );
            }
            ...
            if( errCode == LETO_CLIENT_LOCKON )
            {
               ...
               errCode = leto_BackupStart( fClose, iDelay, pMsgLines );
               /* pMsgLines free-ed in leto_BackupStart() */
            }
            else
            {
...
static void leto_BackupBox( int iTop, int iLeft, int iBottom, int iRight, HB_BOOL fWait, PHB_ITEM pMsgLines )
{
   char    szOldColor[ HB_CLRSTR_LEN ];
   HB_SIZE nPos = 1, nLen = pMsgLines ? hb_arrayLen( pMsgLines ) : 0;
   ...
   while( nLen > 2 && nPos <= nLen - 2 )
   {
      iMsgLen = ( int ) hb_arrayGetCLen( pMsgLines, nPos );
      hb_gtWriteAt( ++iTop, iLeft + 1 + ( ( iWidth - iMsgLen ) / 2 ),
                    hb_arrayGetCPtr( pMsgLines, nPos ), iMsgLen );
      nPos++;
   }
   hb_gtSetColorStr( "B/G" );
   if( nPos < nLen && ! fWait )  /* different lines for GO/QUIT */
      nPos = nLen;
   if( nPos <= nLen )
   {
      const char * pEscPos = strstr( hb_arrayGetCPtr( pMsgLines, nPos ), "ESC" );

      iMsgLen = ( int ) hb_arrayGetCLen( pMsgLines, nPos );
      hb_gtWriteAt( --iBottom, iLeft + 1 + ( ( iWidth - iMsgLen ) / 2 ),
                    hb_arrayGetCPtr( pMsgLines, nPos ), iMsgLen );
      if( pEscPos )
         hb_gtSetPos( iBottom, iLeft + 2 + ( ( iWidth - iMsgLen ) / 2 ) +
                      pEscPos - hb_arrayGetCPtr( pMsgLines, nPos ) );
      else
         hb_gtSetPos( iBottom, iLeft + 1 + ( ( iWidth - iMsgLen ) / 2 ) );
   }

   hb_gtSetColorStr( szOldColor );
}

static HB_ERRCODE leto_InfoLocks( LETOCONNECTION * pConnection, char cRequest )
{
   HB_ERRCODE    errCode = HB_SUCCESS;
   char          szData[ 16 ];

   eprintf( szData, "%c;locks;%c;", LETOCMD_admin, cRequest );
   if( ! leto_SendRecv2( pConnection, szData, 10, 1021 ) )
      errCode = HB_FAILURE;

   return errCode;
}

static void leto_ReleaseWAEnv( PHB_ITEM pSavedLocks )
{
   HB_SIZE  nPos = 0;
   PHB_ITEM pSubArr;
   ...
   if( pSavedLocks )
   {
      while( ++nPos <= hb_arrayLen( pSavedLocks ) )
      {
         pSubArr = hb_arrayGetItemPtr( pSavedLocks, nPos );
         if( pSubArr && ( hb_arrayGetType( pSubArr, 5 ) & HB_IT_POINTER ) )
            hb_xfree( hb_arrayGetPtr( pSubArr, 5 ) );
      }
...
static HB_ERRCODE leto_BackupStart( HB_BOOL fClose, int iDelay, PHB_ITEM pMsgLines )
{
   ...
   if( pConnection )
   {
      ...
      int      iArea;
      char *   pBuffer;
      const char *   ptrStart = ( pMsgLines && hb_arrayLen( pMsgLines ) ) ? hb_arrayGetCPtr( pMsgLines, 1 ) : NULL;
      const char *   ptrEnd;
      ...
      while( *ptrStart )
      {
         ...
         if( ! ptrEnd )
            ptrEnd = ptrStart + HB_MIN( strlen( ptrStart ), 60 );
         iHeight++;
         iWidth = HB_MAX( iWidth, ptrEnd - ptrStart );
         hb_arraySize( pMsgLines, iHeight + 1 );
         hb_arraySetCL( pMsgLines, iHeight + 1, ptrStart, ptrEnd - ptrStart );
         if( *ptrEnd )
            ptrEnd++;
         ptrStart = ptrEnd;
      }
      if( pMsgLines && hb_arrayLen( pMsgLines ) > 1 )
      {
         hb_arrayDel( pMsgLines, 1 );
         hb_arraySize( pMsgLines, iHeight );
      }
      if( iHeight > 1 )
         iHeight--;
      iTop = iYMiddle - ( int ) ( iHeight / 2 ) - 1;
      iBottom = iYMiddle + ( int ) ( iHeight / 2 ) + 1;
      iLeft = iXMiddle - ( int ) ( iWidth / 2 ) - 1;
      iRight = iXMiddle + ( int ) ( iWidth / 2 ) + 1;
      ...
      while( iDelay > 0 )
      {
         ...
         if( ( int ) ( leto_MilliDiff( llLastAct ) / 1000 ) >= iDelay )
            break;
         else if( pConnection->uiBackupActive > 0 )
            break;
      }
      if( fContinueWork )
      {
         ...
         leto_BackupBox( iTop, iLeft, iBottom, iRight, HB_FALSE, pMsgLines );
      hb_itemRelease( pMsgLines );
      ...
      if( pConnection->uiBackupActive < 2 )
      {
         ...
         leto_InfoLocks( pConnection, 'F' );
         /* wait before proceeding, that all have restored their WA env */
         while( ! errCode && pConnection->uiBackupActive )
         {
            /* removed inkey to exit, wait for: LETO_CLIENT_LOCKOFF */
            hb_releaseCPU();
            hb_releaseCPU();
            delayedError();
         }
...
static void leto_ipInit( void )
{
   ...
   else
   {
      ...
      WSADATA wsadata;
      WSAStartup( HB_MKUSHORT( 1, 1 ), &wsadata );
   #endif
#endif
      s_iSessions = 1;
   }
      ...
      if( hb_socketInetAddr( &pSockAddr, &uiLen, pszIpAddres, iPort ) )
      {
         ...
         if( hb_socketConnect( sd, pSockAddr, uiLen, iTimeOut ) == 0 )
         {
            ...
            if( pConnection && ! pConnection->pAddrDNS && strcmp( pszIpAddres, szHost ) )
               pConnection->pAddrDNS = leto_strdup( szHost );
         }
      }
...
static long leto_Send( LETOCONNECTION * pConnection, const char * szData, unsigned long ulLen )
{
   ...
   {
#if ! defined( __HARBOUR30__ ) && ! defined( __LETO_C_API__ )  /* new function since 2015/08/17 */
      if( iErr )
...
HB_BOOL leto_getIpFromPath( const char * szSource, char * szAddr, int * piPort, char * szPath )
{
   ...
   while( iLen >= 5 )  /* "//1.1.1.1/" len 10-18 or "//lh/" len 5-66 */
   {
      ...
      if( atoi( szAddr ) == 0 )  /* instead a valid hostname ? */
      {
         ...
         if( pFoundConn )
         {
            szIP = leto_strdup( pFoundConn->pAddr );
#ifdef LETO_CLIENTLOG
         ...
         else
         {
            ...
            {
               pFoundConn->pAddrDNS = leto_strdup( szAddr );
#ifdef LETO_CLIENTLOG
...
int leto_Connect( const char * szAddress, const char * szUser, const char * szPass, int iTimeOut, int iRefr, HB_BOOL fZombieCheck )
{
   ...
   if( szAddress )
      leto_strncpy( szPath, szAddress, 95 );
...
static void LetoDbFreeTag( LETOTAGINFO * pTagInfo )
{
   if( pTagInfo->BagName )
...
static void letoDbClearTagInfos( LETOTABLE * pTable )
{
...
const char * leto_ParseTagInfo( LETOTABLE * pTable, const char * pBuffer )
{
   ...
   while( *ptr && *ptr++ != ';' )
      ;
   if( pTable->pTagInfo && pTable->uiOrders > uiOrders )  /* safety - should not happen */
      letoDbClearTagInfos( pTable );
...
void leto_AddTagInfo( LETOTABLE * pTable, const char * szTag, unsigned int uiFlags, const char * szWhile, const char * szUseIndex )
{
   ...
   while( pTagInfo )
   {
      ...
      {
         if( szWhile )
            pTagInfo->WhileExpr = leto_strdup( szWhile );
         if( szUseIndex )
            pTagInfo->UseIndex = leto_strdup( szUseIndex );
...
{
   const char * ptr2 = ptr ? strchr( ptr, ';' ) : NULL;
...
void LetoConnectionOpen( LETOCONNECTION * pConnection, const char * szAddr, int iPort, const char * szUser, const char * szPass, int iTimeOut, HB_BOOL fZombieCheck )
{
   ...
   #ifndef __XHARBOUR__
      #ifdef LETO_CLIENTLOG
                     leto_clientlog( NULL, 0, "DEBUG: multithread want %d can %d", ( int ) fZombieCheck, ( int ) hb_vmIsMt() );
      #endif
...
HB_ERRCODE LetoDbCloseAll( LETOCONNECTION * pConnection )
{
   char szData[ 4 ];
...
/* unused */
unsigned int LetoConnectionInfo( LETOCONNECTION * pConnection, unsigned int * iError, unsigned int * iZipLevel, char * szVersion )
{
   if( ! pConnection )
      return 1;
   ...
   if( iError )
   {
      *iError = pConnection->iError;
   #ifndef LETO_NO_THREAD
      if( ! pConnection->iError && pConnection->hSocketErr != HB_NO_SOCKET && s_pError )
         *iError = 1111;
   #endif
   }
   if( iZipLevel )
      *iZipLevel = pConnection->iZipRecord;
   if( szVersion )
      leto_strncpy( szVersion, pConnection->szVersion, 23 );
...
static const char * leto_AddFields( LETOTABLE * pTable, HB_USHORT uiFields, const char * szFields )
{
   ...
   for( uiCount = 0; uiCount < uiFields; uiCount++ )
   {
      ...
      if( uiLen > ( unsigned int ) pTable->uiFieldNameLength )
      {
         uiLen = HB_MIN( uiLen, HB_SYMBOL_NAME_LEN );
         pTable->uiFieldNameLength = HB_SYMBOL_NAME_LEN;
      }
...
{
   leto_strncpy( s_szSMBServer, szServer, 41 );
   s_iSMBPort = iPort;
   if( pConnection )
...
LETOTABLE * LetoDbCreateTable( LETOCONNECTION * pConnection, const char * szFile, const char * szAlias,
                               const char * szFields, unsigned int uiArea, const char * szCdpage, HB_BOOL fTemporary )
{
   ...
   unsigned long ulLen;
   char          szNewAlias[ HB_RDD_MAX_ALIAS_LEN + 1 ];
   ...
   {
      if( ! szFile || ! *szFile )
         eprintf( szNewAlias, "TmpWA%u", uiArea );
      else
         LetoDbCreateAlias( szFile, szNewAlias );
   }
   else
      leto_strncpy( szNewAlias, szAlias, HB_RDD_MAX_ALIAS_LEN );

   ulLen = eprintf( szData, "%c;%s;%s;%s;%d;%s;%d;%d;%s;%d;%s;%s;%c;", LETOCMD_creat, szFile,
                    szNewAlias, pCallerConn->szDriver,
   ...
   strcpy( pTable->szAlias, szNewAlias );
...
/* unused */
unsigned int LetoDbAreaFlags( LETOTABLE * pTable, unsigned int * uiFlags )
{
   if( ! pTable || ! uiFlags )
      return 1;
...
unsigned int LetoDbGetField( LETOTABLE * pTable, HB_USHORT uiIndex, char * szRet, unsigned long * ulLen )
{
   LETOFIELD * pField;
   HB_ULONG    ulFldLen;
   HB_BOOL     fFullContent = HB_TRUE;

   if( ! pTable || ! ulLen || ! *ulLen || ! uiIndex || uiIndex > pTable->uiFieldExtent )
   {
      if( ulLen && *ulLen && szRet )
         *szRet = '\0';
      if( ulLen )
         *ulLen = 0;
      return 1;
   ...
   {
      const char * szResult = LetoDbGetMemo( pTable, uiIndex, ( unsigned long * ) &ulFldLen );

      if( szResult )
      {
         if( *ulLen < ulFldLen + 1 )
         {
            fFullContent = HB_FALSE;
            ulFldLen = *ulLen - 1;
         }
...
/* unused */
unsigned int LetoDbGetFieldLen( LETOTABLE * pTable, HB_USHORT uiIndex, unsigned long * ulContentLen )
{
   LETOFIELD * pField;

   if( ! pTable || ! uiIndex || uiIndex > pTable->uiFieldExtent )
      return 1;
       ...
       pField->uiType == HB_FT_PICTURE || pField->uiType == HB_FT_OLE )
   {
      ...
      if( leto_SendRecv( pConnection, szData, ulLen, 0 ) )
      {
         if( *pConnection->szBuffer == '+' )
         {
            *ulContentLen = strtoul( pConnection->szBuffer + 1, NULL, 10 );
            return 0;
         }
      }
      return 1;
      ...
      *ulContentLen = ( pTable->pFields + uiIndex - 1 )->uiLen;
...
unsigned int LetoDbPutField( LETOTABLE * pTable, HB_USHORT uiIndex, const char * szValue, unsigned long ulLen )
{
   ...
   if( ! ulLen || ulLen > ( unsigned long ) pField->uiLen )  /* poss. binary ! */
      return 1;
   ...
   else if( pField->uiType == HB_FT_LONG || pField->uiType == HB_FT_FLOAT )
   {
...
/* unused */
unsigned int LetoDbAlias( LETOTABLE * pTable, char * szAlias )
{
   if( ! pTable || ! szAlias )
      return 1;

   strcpy( szAlias, pTable->szAlias );
...
/* unused */
unsigned int LetoDbFieldInfo( LETOTABLE * pTable, HB_USHORT uiIndex, char * szInfoList )
{
   const char * cTypes = " CLDNFIBT@=^+YZQMVPWG?";
   LETOFIELD *  pField;

   if( ! pTable || ! uiIndex || uiIndex > pTable->uiFieldExtent || ! szInfoList  )
      return 1;

   pField = pTable->pFields + uiIndex - 1;
   if( pField->uiType < 21 )
   {
       eprintf( szInfoList, "%s,%c,%u,%u", pField->szName, cTypes[ pField->uiType ], pField->uiLen, pField->uiDec );
       return 0;
   }
...
/* unused */
unsigned int LetoDbFieldNameLength( LETOTABLE * pTable, unsigned int * uiLength )
{
   if( ! pTable || ! uiLength )
      return 1;

   *uiLength = pTable->uiFieldNameLength;
   return 0;
}

/* unused */
unsigned int LetoDbFieldPos( LETOTABLE * pTable, const char * szField, unsigned int * uiIndex )
{
   HB_USHORT uiPos;

   if( ! pTable || ! szField || ! *szField )
      return 1;

   uiPos = 0;
   while( uiPos < pTable->uiFieldExtent )
   {
      if( ! strcmp( szField, ( pTable->pFields + uiPos++ )->szName ) )
      {
         *uiIndex = uiPos;
         return 0;
      }
                         ...
                         HB_LONG lNext, HB_LONG lRecNo, HB_LONG lRest )
{
   ...
   if( pConnection )
   {
      ...
      if( strncmp( pConnection->szBuffer, "-00", 3 ) )
      {
         ...
         return *pConnection->szBuffer == '+' ? HB_SUCCESS : HB_FAILURE;
      }
   }
...
/* unused */
unsigned int LetoDbSetRelation( LETOTABLE * pTable, HB_USHORT uiChildArea, const char * szRelation )
{
   LETOCONNECTION * pConnection = letoGetConnPool( pTable->uiConnection );

   if( pConnection && pConnection->uiServerMode >= 3 && szRelation )
   {
      char *        pData = ( char * ) hb_xgrab( strlen( szRelation ) + 32 );
      unsigned long ulLen = eprintf( pData, "%c;%lu;01;%u;%s;", LETOCMD_rela, pTable->hTable,
                                            uiChildArea, szRelation );
      if( ! leto_SendRecv( pConnection, pData, ulLen, 1026 ) )
      {
         hb_xfree( pData );
         return 1;
...
/* unused */
unsigned int LetoDbClearRelation( LETOTABLE * pTable )
{
   LETOCONNECTION * pConnection = letoGetConnPool( pTable->uiConnection );
   ...
   {
      char     szData[ 24 ];
      HB_ULONG ulLen = eprintf( szData, "%c;%lu;02;", LETOCMD_rela, pTable->hTable );

      if( leto_SendRecv( pConnection, szData, ulLen, 1026 ) )
         return 0;
   }
                                        ...
                                        const char * szFor, const char * szWhile, unsigned long ulNext )
{
   ...
   if( ( uiFlags & LETO_INDEX_USEI ) && pTable->pTagCurrent )
   {
      ...
      pUseIndex = leto_strdup( pTable->pTagCurrent->TagName );
   }
...
HB_ERRCODE LetoDbOrderDestroy( LETOTABLE * pTable, const char * szTagName, const char * szBagName )
{
   LETOCONNECTION * pConnection = letoGetConnPool( pTable->uiConnection );
   char          szTag[ LETO_MAX_TAGNAME + 1 ];
   char          szData[ HB_PATH_MAX ];
   unsigned long ulLen;
   ...
   leto_strncpy( szTag, szTagName, LETO_MAX_TAGNAME );
   ulLen = eprintf( szData, "%c;%lu;13;%s;%s;", LETOCMD_ord, pTable->hTable, szTag, szBagName );
   ulLen = leto_SendRecv( pConnection, szData, ulLen, 0 );
   if( ! ulLen || *pConnection->szBuffer != '+' )
      return HB_FAILURE;

   leto_ParseTagInfo( pTable, leto_firstchar( pConnection ) );
   pTable->ptrBuf = NULL;

   return HB_SUCCESS;
}

HB_ERRCODE LetoDbOrderListAdd( LETOTABLE * pTable, const char * szIFileName )
{
   LETOCONNECTION * pConnection = letoGetConnPool( pTable->uiConnection );
   unsigned long ulLen;
   ...
   if( ! szIFileName || ! *szIFileName )
      return HB_FAILURE;
   if( pTable->uiUpdated )
      LetoDbPutRecord( pTable );

   leto_strncpy( szBagName, szIFileName, HB_PATH_MAX - 1 );
   leto_BeautifyPath( szBagName, 0 );

   ulLen = eprintf( szData, "%c;%lu;%s;", LETOCMD_open_i, pTable->hTable, szBagName );
   ulLen = leto_SendRecv( pConnection, szData, ulLen, 0 );
   if( ! ulLen || *pConnection->szBuffer != '+' )
      return HB_FAILURE;

   ptr = leto_ParseTagInfo( pTable, leto_firstchar( pConnection ) );
   if( ulLen >= 4 && ulLen - 4 > ( unsigned long ) ( ptr - pConnection->szBuffer ) )
   {
      leto_ParseRecord( pConnection, pTable, ptr );
      if( pTable->fAutoRefresh )
         pTable->llCentiSec = leto_MilliSec();
...
HB_ERRCODE LetoDbOrderListDelete( LETOTABLE * pTable, const char * szIFileName )
{
   LETOCONNECTION * pConnection = letoGetConnPool( pTable->uiConnection );
   char          szData[ HB_PATH_MAX + 16 ];
   char          szBagName[ HB_PATH_MAX ];
   ...
   ulLen = eprintf( szData, "%c;%lu;10;%s;", LETOCMD_ord, pTable->hTable, szBagName );
   ulLen = leto_SendRecv( pConnection, szData, ulLen, 0 );
   if( ! ulLen || *pConnection->szBuffer != '+' )
      return HB_FAILURE;
...
HB_ERRCODE LetoDbOrderListClear( LETOTABLE * pTable )
{
   LETOCONNECTION * pConnection = letoGetConnPool( pTable->uiConnection );
   ...
   if( pTable->pTagInfo )
   {
      ...
      ulLen = eprintf( szData, "%c;%lu;04;", LETOCMD_ord, pTable->hTable );
      if( ! leto_SendRecv( pConnection, szData, ulLen, 0 ) || *pConnection->szBuffer != '+' )
         return HB_FAILURE;
      leto_ParseTagInfo( pTable, leto_firstchar( pConnection ) );
      pTable->ptrBuf = NULL;
...
/* unused */
const char * LetoDbOrder( LETOTABLE * pTable, unsigned int uiNameKeyTagBag )
{
   if( pTable && pTable->pTagCurrent )
   {
      switch( uiNameKeyTagBag )
      {
         case 0:
            return pTable->pTagCurrent->TagName;
         case 1:
            return pTable->pTagCurrent->KeyExpr;
         case 2:
            return pTable->pTagCurrent->TagName;
         case 3:
            return pTable->pTagCurrent->BagName;
      }
   }
...
/* unused */
unsigned int LetoDbOrdCount( LETOTABLE * pTable, unsigned int * iOrdCount )
{
   LETOTAGINFO * pTag;
   int           iCount = 0;
...
/* unused */
unsigned int LetoDbOrdInfo( LETOTABLE * pTable, HB_BOOL fActive, char * szOrdInfo, unsigned int uiAlloc )
{
   LETOTAGINFO * pTag;
   unsigned int  iNew, iRes = 0;
   char *        ptr;
   unsigned int  iCurrent = 0, iMax = 0;
   char          cTmpInfo[ LETO_MAX_TAGNAME + LETO_MAX_KEY + 4 + 13 ];

   if( ! pTable || ! szOrdInfo || uiAlloc < 13 )
      return 1;
   ...
   {
      iMax++;
      if( pTag == pTable->pTagCurrent )
         iCurrent = iMax;
      pTag = pTag->pNext;
   }
   ptr += eprintf( ptr, "%u;%u;", iCurrent, iMax );
   ...
   {
      iMax++;
      if( ! fActive || iCurrent == iMax )
      {
         iNew = eprintf( cTmpInfo, "%s;%c;%s;", pTag->TagName, pTag->cKeyType, pTag->KeyExpr );
         if( iNew + 1 + ( ptr - szOrdInfo ) <= uiAlloc )
         {
            memcpy( ptr, cTmpInfo, iNew + 1 );  /* incl. '\0' */
            ptr += iNew;
         }
         else
         {
...
/* unused */
unsigned int LetoVarGetC( LETOCONNECTION * pConnection, const char * szGroup, const char * szVar, void * xValue, unsigned long * pulLen )
{
   unsigned long ulLen;
   const char *  szRaw = LetoVarGet( pConnection, szGroup, szVar, &ulLen );
   ...
   if( szRaw )
   {
      switch( szRaw[ 0 ] )
      {
         case LETOVAR_STR:
         {
            /* one in reserve for temination */
            unsigned long ulAlloc = ( pulLen && *pulLen ) ? *pulLen - 1 : 0;
            unsigned long ulToDo = HB_MIN( ulAlloc ? ulAlloc - 1 : 0, ulLen );

            if( ulToDo )
            {
               /* there are 2 bytes length leading */
               memcpy( ( char * ) xValue, szRaw + 2, ulToDo );
               ( ( char * ) xValue )[ ulToDo ] = '\0';
               if( pulLen )
                  *pulLen = ulToDo;
            }
            if( ! ulToDo || ulToDo + 1 > ulAlloc )
               return 0;
            break;
         }
         ...
         case LETOVAR_NUM:
            if( strstr( szRaw + 2, "." ) )
               *( (  double * ) xValue ) = strtod( szRaw + 2, NULL );
            else
               *( ( long * ) xValue ) = strtol( szRaw + 2, NULL, 0 );
            break;

         case LETOVAR_DAT:
            /* *( ( long * ) xValue ) = hb_dateEncStr( szRaw + 2 ); */
            memcpy( ( char * ) xValue, szRaw + 2, 8 );
            ( ( char * ) xValue )[ 8 ] = '\0';
            break;

         case LETOVAR_LOG:
            *( ( int * ) xValue ) = atoi( szRaw + 2 );
            break;

         default:  /* LETOVAR_ARR is HB specific with any (mixed) types */
            return 0;
      }
   }
...
HB_BOOL LetoFileExist( LETOCONNECTION * pConnection, const char * szFile, char ** pRetPath )
{
   ...
   if( uiRes )
   {
      ...
      if( *( ptr - 1 ) == '+' && *ptr == 'T' )
      {
         ...
         {
            leto_strncpy( *pRetPath, ptr + 2, HB_PATH_MAX - 1 );
            while( *( *pRetPath + nPos ) )
            {
...
int LetoToggleZip( LETOCONNECTION * pConnection, int iZipRecord, const char * szPassword )
{
   ...
   if( pConnection && iZipRecord >= -1 && ( pConnection->fCryptTraf ? pConnection->iZipRecord < 0 : HB_TRUE ) )
   {
...
#endif
      {
         ...
         eprintf( szData, "%c;%d;%s;", LETOCMD_zip, iZipRecord, szPass );
         hb_xfree( szPass );
...
HB_BOOL LetoUdf( LETOCONNECTION * pConnection, LETOTABLE * pTable, HB_BOOL fInThread, const char * szFuncName, PHB_ITEM * pItem )
{
   ...
   if( pConnection )
   {
      ...
      if( fLetoArea )  /* valid LETOAREA pArea */
         eprintf( szData, "%c;%lu;%c;%c;%lu;%s;%c;%lu;", LETOCMD_udf_dbf, pTable->hTable,
                  fInThread ? '9' : '1', ( char ) ( ( hb_setGetDeleted() ) ? 0x41 : 0x40 ),
                  pTable->ulRecNo, szFuncName, fExclusive ? 'T' : 'F', nSize );
      else
         eprintf( szData, "%c;%c;%c;%lu;%s;%c;%lu;", LETOCMD_udf_fun,
      ...
      if( leto_DataSendRecv( pConnection, szData, ( ptr - szData ) + nSize ) )
      {
         ...
         else if( ptr[ 0 ] == '+' )  /* NEW: pre-leading '+' before valid [ zero ] result */
         {
            ...
            if( pItem )
            {
               ...
               if( ulLen )
               {
                  pResult = hb_itemDeserialize( &ptrTmp, ( HB_SIZE * ) &ulLen );
                  hb_itemSetNil( *pItem );
                  if( ! HB_IS_NIL( pResult ) )
                     hb_itemCloneTo( *pItem , pResult );
                  else
