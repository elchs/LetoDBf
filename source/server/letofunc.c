...
HB_FUNC( LETO_SETAPPOPTIONS )  /* during server startup */
{
   ...
   {
      s_pDataPath = ( char * ) hb_xgrab( hb_parclen( 1 ) + 1 );
      if( hb_parclen( 1 ) )
   ...
   if( HB_ISLOG( 25 ) )
   {
      ...
      if( s_bLowerPath )
         hb_strLower( s_pDataPath, strlen( s_pDataPath ) );
   }
   if( HB_ISCHAR( 26 ) )
   {
...
static void leto_DataPath( const char * szFilename, char * szBuffer )
{
   ...
   if( szFilename[ 0 ] )
   {
      ...
      leto_StrTran( szBuffer, DEF_CH_SEP, DEF_SEP, strlen( szBuffer ) );
   }
}
...
static HB_ULONG leto_rec( PUSERSTRU pUStru, PAREASTRU pAStru, AREAP pArea, char * szData, HB_ULONG ulLenData, HB_BOOL fRefr )
{
   ...
   if( uiFieldCount )
   {
      ...
      if( fRefr && s_bShareTables && pAStru->pTStru->bShared )
...
static PGLOBESTRU leto_InitGlobe( const char * szTable, HB_U32 uiCrc, HB_UINT uiTable, HB_USHORT uiLen )
{
   PGLOBESTRU pGStru;
...
static int leto_InitTable( HB_ULONG ulAreaID, const char * szName, const char * szDriver, HB_BOOL bShared, const char * szLetoAlias )
{
   ...
   pTStru->pGlobe = leto_InitGlobe( szName, pTStru->uiCrc, uiTable, uiLen );
...
static HB_ULONG leto_CryptText( PUSERSTRU pUStru, const char * pData, HB_ULONG ulLen )
{
   ...
   if( ulBufLen > pUStru->ulBufCryptLen )
   {
      if( ! pUStru->ulBufCryptLen )
      {
         ...
         pUStru->pBufCrypt = ( HB_BYTE * ) hb_xgrab( pUStru->ulBufCryptLen + 1 );
         if( s_iDebugMode > 10 )
            leto_writelog( NULL, -1, "DEBUG crypt-buffer size %lu", pUStru->ulBufCryptLen );
      }
      else
      {
      ...
      while( pTStru )
      {
         ...
         if( ! pTStru->bMemIO && hb_arrayScan( pTableFiles, pFileName, NULL, NULL, HB_TRUE ) < 1 )
         {
            ...
            if( ! bDryRun && ( pTStru->bShared || ( ! pTStru->bShared && ! pTStru->bWhoCares ) ) )
            {
               ...
               HB_USHORT  uiLen = ( HB_USHORT ) strlen( s_pDataPath );
               ...
               if( pArea )
               {
                  ...
                  if( leto_DirMake( szTarget, HB_TRUE ) && leto_dbfCopy( pArea->pDataFile, szTarget, 0 ) )
                  {
                     ...
                     if( ( ui = ( HB_USHORT ) hb_arrayLen( pIndexFiles ) ) > 0 )
                     {
                        ...
                        memset( &pOrderInfo, 0, sizeof( DBORDERINFO ) );
                        uiLen = ( HB_USHORT ) strlen( s_pDataPath );
               ...
               if( pUStru1->iUserStru )  /* ->hThread */
               {
                  ...
                  if( bShow )
                  {
                     ...
                     if( uiUsers >= iListStart )
                     {
                        ...
                        //ptr += sprintf( ptr, "%d;%s;%s;%s;%" HB_PFS "d;%s;%d;%s;%d;",
...
static void leto_Intro( PUSERSTRU pUStru, const char * szData )
{
   ...
   else
   {
      ...
      if( ! pData && nParam > 5 )
      {
         ...
         if( pp5 && *pp5 )  /* terminated by leto_GetParam() */
         {
...
static void leto_CloseT( PUSERSTRU pUStru, const char * szData )  //mt
{
   const char * pData;
   HB_BOOL      bOk = HB_FALSE;
...
static void leto_OpenTable( PUSERSTRU pUStru, const char * szRawData )
{
   ...
   /* szRawData = filename, pp2 = Alias, pp3 = shared/readonly, pp4 = codepage, pp5 = DBdriver */
   nParam = leto_GetParam( szRawData, &pp2, &pp3, &pp4, &pp5 );
   ...
   else
   {
      ...
      if( ! pp4 || ! *pp4 )  /* important: valid str-len or NULL ! */
         pp4 = NULL;
